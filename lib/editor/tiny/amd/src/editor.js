...
/**
 * Store for RTL keyboard workaround flag.
 * @type {boolean}
 */
let rtlKeyboardWorkaroundInstalled = false;
    ...
    const config = Object.assign({}, getDefaultConfiguration(), {
        ...
        setup: (editor) => {
            ...
            editor.on('init', function() {
                ...
                // Workaround for RTL mode keyboard navigation in submenus.
                // MDL-86812: In RTL mode, the left/right arrow keys are inverted in TinyMCE submenus.
                const isRTL = document.documentElement.dir === 'rtl' || document.body.dir === 'rtl';
                if (isRTL && !rtlKeyboardWorkaroundInstalled) {
                    rtlKeyboardWorkaroundInstalled = true;

                    const patchTinyMCEKeying = () => {
                        // Find the silver sink container where menus are rendered.
                        const silverSink = document.querySelector('.tox-silver-sink');
                        if (!silverSink) {
                            // Retry after a short delay if not found yet.
                            setTimeout(patchTinyMCEKeying, 100);
                            return;
                        }

                        // Add a capturing event listener on the silver sink to intercept keyboard events.
                        silverSink.addEventListener('keydown', function(e) {
                            // Only handle left/right arrow keys.
                            if (e.keyCode !== 37 && e.keyCode !== 39) {
                                return;
                            }

                            // Skip if in menubar or toolbar - these work correctly.
                            const isInMenubar = e.target.closest('.tox-menubar');
                            const isInToolbar = e.target.closest('.tox-toolbar') ||
                                              e.target.closest('.tox-toolbar__primary') ||
                                              e.target.closest('.tox-toolbar__overflow');

                            if (isInMenubar || isInToolbar) {
                                return;
                            }

                            // Check if we're in a dropdown menu/collection.
                            const isInMenu = e.target.closest('.tox-menu') ||
                                           e.target.closest('.tox-collection') ||
                                           e.target.closest('.tox-tree');

                            if (!isInMenu) {
                                return;
                            }

                            const currentItem = e.target.closest('.tox-collection__item');
                            if (!currentItem) {
                                return;
                            }

                            // Check if item has a submenu indicator (caret).
                            const hasCaret = currentItem.querySelector('.tox-collection__item-caret');

                            // Check if we're inside a nested submenu (not the primary dropdown).
                            const allMenus = Array.from(silverSink.querySelectorAll('.tox-menu'));
                            const currentMenu = currentItem.closest('.tox-menu');
                            const isInSubmenu = allMenus.indexOf(currentMenu) > 0;

                            // TinyMCE doesn't respect RTL for submenus - it always uses LTR logic.
                            // Always swap arrow keys in submenu contexts (items with submenus or inside submenus).
                            // This correctly handles opening/closing submenus and navigation within them.
                            // Trade-off: Navigation between top-level menus from submenu items may go the "wrong way",
                            // but users can navigate via the menubar instead.
                            if (!hasCaret && !isInSubmenu) {
                                // Top-level item without submenu - normal navigation works.
                                return;
                            }

                            // Swap the arrow key codes.
                            const swappedKeyCode = e.keyCode === 37 ? 39 : 37;
                            ...
                            try {
                                ...
                                // Properties might not be configurable - workaround won't work in this case.
                            }
                        }, true); // Use capture phase to intercept before TinyMCE's handlers.
                    };
