...
export interface SecondaryNavActionLinkAction {
    id: string;
    event: string;
    jsfunction: string;
 ...
 */
const useActionLinkBehavior = (items: SecondaryNavNode[]): void => {
    useEffect(() => {
...
/**
 * A tab pill rendered for an `istablist` secondary nav (e.g. admin/search.php's site-admin
 * category tabs).
 *
 * @param props Component props.
...
/**
 * Root component for the secondary navigation pill.
 *
 * @param props Component props.
...
export default function SecondaryNav({items, morelabel, istablist}: SecondaryNavProps) {
    ...
    // intermediate states are never visible; MEASURED_CLASS/opacity cover the first convergence.
    useLayoutEffect(() => {
